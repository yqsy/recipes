...
func Usage() {
  ...
  fmt.Fprintln(os.Stderr, "  SolveReply Solve(SolveRequest solveRequest)")
  fmt.Fprintln(os.Stderr)
  os.Exit(0)
}
...
func main() {
  ...
  case "Solve":
    if flag.NArg() - 1 != 1 {
      fmt.Fprintln(os.Stderr, "Solve requires 1 args")
      flag.Usage()
    }
    arg4 := flag.Arg(1)
    ...
    argvalue0 := sudoku_protocol.NewSolveRequest()
    ...
    value0 := argvalue0
    fmt.Print(client.Solve(context.Background(), value0))
    fmt.Print("\n")
    break
  case "":
    Usage()
